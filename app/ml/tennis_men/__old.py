...
class StandardTennisMenML:
    ...
    def save_model(self, model):
        with open(DIST_DIR / MODEL_FILENAME, "wb") as f:
            pickle.dump(model, f)

    def upload_model(self):
        with open(DIST_DIR / MODEL_FILENAME, "rb") as f:
...
class StandardTennisMenTrainer(StandardTennisMenML):
    ...
    def setup_matches_features(
        ...
        features_df.to_excel(DIST_DIR / PREPROCESSED_FEATURES_FILENAME, index=False)
    ...
    async def train(
        ...
        if preprocessed_features:
            if not os.path.exists(DIST_DIR / PREPROCESSED_FEATURES_FILENAME):
                raise FileExistsError("Preprocessed features file does not exist")
            features_df = pd.read_excel(DIST_DIR / PREPROCESSED_FEATURES_FILENAME)
        else:
        ...
        if preprocessed_na_filler:
            if not os.path.exists(DIST_DIR / NA_FILLER_FILENAME):
                raise FileExistsError("NA Filler file does not exist")
            na_filler = pd.read_excel(DIST_DIR / NA_FILLER_FILENAME)
        else:
...
class StandardTennisMenPredictor(StandardTennisMenML):
    def __init__(
        self,
        sport: SportType,
        data: TennisMenData,
    ...
    def initialize(self) -> None:
        if not os.path.exists(DIST_DIR / NA_FILLER_FILENAME):
            raise FileExistsError("NA Filler file does not exist")
        if not os.path.exists(DIST_DIR / MODEL_FILENAME):
            raise FileExistsError("Model file does not exists")

        self.na_filler = pd.read_excel(DIST_DIR / NA_FILLER_FILENAME, index_col=0)
